...
- No middleware / proxy.ts needed (no auth routing)
...
- RLS policies allow public read/insert (admin page has no auth)

## Pages Structure
- `/` — Voting page (select year, vote on courses)
- `/dashboard` — Vote summary with year filtering
- `/admin` — Admin CRUD for courses + year restrictions (no login)
